<template>
  <div class="cells" :class="theme">
    <div class="row" v-for="(r, index) in rows">
      ...
      <div class="col" v-for="c in cols" @click="cellClicked(r*cols.length + c, $event)" :class="{'active': ((r*cols.length + c)==selected)}">
...
<script>
  ...
  export default {
    ...
    methods: {
      cellClicked(cellIndex, event) {
//        console.log(cellIndex, event);
